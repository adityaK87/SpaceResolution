    ...
    if($('.super-shortcode.super-field.super-recaptcha:not(.super-rendered)').length){
        if( (typeof grecaptcha === 'undefined') || (typeof grecaptcha.render === 'undefined') ) {
            if(!SUPER.reCaptchaScriptLoaded){
                $.getScript( '../../www.google.com/recaptcha/api63e8.js?onload=SUPERreCaptcha&amp;render=explicit', function() {
...
(function($) {
    ...
    SUPER.get_single_uploaded_file_html = function(withoutHeader, uploaded, fileName, fileType, fileUrl){
        ...
        if(fileType && fileType.indexOf("image/index.html") === 0){
            html += '<span class="super-fileupload-image super-file-type-'+fileType.replace('../index.html','-')+'">';
                html += '<img src="'+fileUrl+'" />';
            html += '</span>';
        }else{
            html += '<span class="super-fileupload-document super-file-type-'+fileType.replace('../index.html','-')+'"></span>';
        }
                ...
                $.each(data.files, function (index, file) {
                    ...
                    if( (total>upload_limit) && (upload_limit!==0) ) {
                        ...
                        if( (file_types_object.indexOf(f.ext)!=-1) || (accepted_file_types==='') ) {
                            ...
                            if(file.type && file.type.indexOf("image/index.html") === 0){
